...
var App = function () {
    ...
    var handlePortletTools = function () {
        ...
        jQuery('.portlet .actions .fullscreen').live('click', function () {
            
            var portlet = $(this).closest(".portlet");
           
            if (portlet.hasClass('portlet-fullscreen')) {
                console.log('tidak full screen');
                $('.btn-add').css('visibility','visible');
                $(this).removeClass('on');
                portlet.removeClass('portlet-fullscreen');
                $('body').removeClass('page-portlet-fullscreen'); 
                $('body').addClass('fixed-top');
                $('body').addClass('breakpoint-1280');
                portlet.children('.portlet-body').css('height', 'auto');
                //portlet.children('.portlet-body').css('padding', '0px');
            } else {
                $('.btn-add').css('visibility','hidden');
                var height = App.getViewPort().height -
                    portlet.children('.portlet-title').outerHeight() -70;
                console.log('full screen');
                $(this).addClass('on');
                portlet.addClass('portlet-fullscreen');
                $('body').addClass('page-portlet-fullscreen');
                $('body').removeClass('fixed-top');
                $('body').removeClass('breakpoint-1280');
                portlet.children('.portlet-body').css('height', height);
                portlet.children('.portlet-body').css('padding','20px');
            }
        });
		...
		jQuery('.portlet .tools a.close').live('click', function () {
			$(this).closest('form').find('input[type=text], textarea').val('');
            var removable = jQuery(this).parents(".portlet");
            if (removable.next().hasClass('portlet') || removable.prev().hasClass('portlet')) {
                jQuery(this).parents(".portlet").slideUp();
            } else {
                jQuery(this).parents(".portlet").parent().slideUp();
            }
        });
    }
...
//
$(".btn-changepwd-submit").live('click', function(){
	var curpwd = $("#password_old").val();
	var newpwd = $("#password_new").val();
	var retpwd = $("#password_retype").val();
	
	if( curpwd=='' ){
		$("#password_old_helper").html("*required field").show();
	}
	
	if( newpwd=='' ){
		$("#password_new_helper").html("*required field").show();
	}
	
	if( retpwd=='' ){
		$("#password_retype_helper").html("*required field").show();
	}
	$('#loading').show();
	$.ajax({
		async: false,
		type: 'post',
		url:  domain + "cms/user/changePassword",
		data: 'pwd=' + curpwd + '&newpwd=' + newpwd + '&retpwd=' + retpwd,
		dataType: 'json',
		success: function(response){
			if(response.code == 200){
				$('#loading').hide();
				$('.successmsg').fadeIn(1000).html(response.message);
                $(".successmsg").fadeOut(4000);
                window.setTimeout(function(){
                    $('#changePWDModal').modal('hide');
                },2500)
			}else{
				$('.errormsg').fadeIn(1000).html(response.message);
...
// notification node js handler
if( typeof nodeProtocol != 'undefined' && typeof nodeHost != 'undefined' && typeof nodePort != 'undefined' && typeof node != 'undefined' && node == 'enable'){
	var socket = io.connect(nodeProtocol+'://'+nodeHost+':'+nodePort);
	...
	socket.on('connect', function (){
		...
		socket.emit('get-notification', { 'uid': userid, 'force':true });
	});
	
	setInterval(function(){
		socket.emit('get-notification', { uid: userid });		
	}, 3000);
	...
	socket.on('notification', function (data) {
		...
		console.log(data);
		if( length != 0 ){
			$("#header_notification_bar .badge").html( response.length );
			$("#header_notification_bar .badge").show();
			$("#header_notification_bar ul li:not(.external)").remove();
			$.each(response, function(i,v){
				var notif = '<li><a href="#"><span class="label label-success"><i class="icon-plus"></i></span>';
	                notif+= v.notification_text + '</a></li>';
				$("#header_notification_bar ul").prepend(notif);
			});
			$("#header_notification_bar ul").prepend('<li><p>You have '+response.length+' new notifications</p></li>');
		}
		else{
